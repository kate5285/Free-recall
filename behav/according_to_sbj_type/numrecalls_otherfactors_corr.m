...
hold on;
a=cell2mat(mean4);
emptyIn=find(cellfun(@isempty,mean4));
...
b=cell2mat(mean5);
emptyIn=find(cellfun(@isempty, mean5));
tmmse=mmse(groupRows{g});
tmmse(emptyIn) = [];
...
c=cell2mat(meann);
emptyIn=find(cellfun(@isempty, meann));
tmmse=mmse(groupRows{g});
tmmse(emptyIn) = [];
...
for g=1:3
   for i=1:3
   figure(4*(g-1)+i+1);
   saveas(gcf,fullfile(savePath,['rmout avg num of recalls_',groupNames{g}, conditionNames{i},'.png']))
   end
end
